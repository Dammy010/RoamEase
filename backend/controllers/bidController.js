...
const {
  createConversation: chatCreateConversation,
} = require("../controllers/chatController"); // New: Import createConversation from chatController
  ...
  try {
    ...
    if (!shipmentId || !price || !eta) {
      return res.status(400).json({
        message:
          "Missing required fields: shipmentId, price, and eta are required",
      });
    }
    ...
    if (req.user.role !== "logistics") {
      return res
        .status(403)
        .json({ message: "Only logistics providers can create bids" });
    }
    ...
    if (existingBid) {
      return res.status(400).json({
        ...
        details:
          "You can only place one bid per shipment. You can edit your existing bid instead.",
      });
    }
    ...
    setImmediate(async () => {
      try {
        ...
        const notificationData = {
          ...
          message: `You received a new bid of ${
            populatedBid.carrier.companyName || populatedBid.carrier.name
          } for your shipment "${shipment.shipmentTitle}".`,
          priority: "high",
          relatedEntity: {
            type: "bid",
            id: bid._id,
        ...
        if (
          conversationRes.statusCode === 201 ||
          conversationRes.statusCode === 200
        ) {
...
const getBidsForShipment = async (req, res) => {
  try {
    ...
    console.log(
      "DEBUG: getBidsForShipment - Requesting bids for shipment ID:",
      shipmentId
    );
    const shipment = await Shipment.findById(shipmentId);
    if (!shipment)
    ...
    console.log(
      "DEBUG: getBidsForShipment - Is shipper (req.user is owner)?:",
    ...
    if (
      ...
      console.log(
        "DEBUG: getBidsForShipment - Authorization failed. User is not the shipment owner and not admin."
      );
      return res
        .status(403)
        .json({ message: "Not authorized to view bids for this shipment" });
    }
    ...
    console.log(
      "DEBUG: getBidsForShipment - Populated Bids with conversations:",
      bidsWithConversations
    );
...
const getMyBids = async (req, res) => {
  try {
    if (req.user.role !== "logistics") {
      return res
        .status(403)
        .json({ message: "Only logistics providers can view their bids" });
    }
    ...
    const bids = await Bid.find({ carrier: req.user._id })
      .populate({
        path: "shipment",
        select:
          "shipmentTitle status pickupAddress pickupCity pickupCountry deliveryAddress deliveryCity deliveryCountry preferredPickupDate preferredDeliveryDate typeOfGoods weight descriptionOfGoods quantity modeOfTransport user createdAt",
        populate: {
          path: "user",
          select:
...
const rejectBid = async (req, res) => {
  try {
    ...
    const populatedBid = await Bid.findById(id)
      ...
      .populate(
        "shipment",
        "shipmentTitle status pickupAddress deliveryAddress"
      );
    ...
    try {
      ...
      console.log(
        "✅ Bid rejection notification sent to logistics company:",
        populatedBid.carrier._id
      );
    } catch (notificationError) {
      console.error(
    ...
    res
      .status(500)
      .json({ message: "Error rejecting bid", error: err.message });
  }
};
...
const restoreBid = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Not authorized to restore this bid" });
    }
    ...
    res
      .status(500)
      .json({ message: "Error restoring bid", error: err.message });
  }
};
...
const getBidsOnMyShipments = async (req, res) => {
  try {
    console.log(
      "DEBUG: getBidsOnMyShipments - Request from user:",
      req.user._id,
      "Role:",
      req.user.role
    );
    ...
    if (
      ...
      console.log(
        "DEBUG: getBidsOnMyShipments - User role not authorized:",
        req.user.role
      );
      return res.status(403).json({
        message:
          "Only shippers, logistics providers, and admins can view bids on their shipments",
      });
    }
    ...
    console.log(
      "DEBUG: getBidsOnMyShipments - User shipments found:",
      userShipments.length
    );
    console.log(
      "DEBUG: getBidsOnMyShipments - Shipment IDs:",
      userShipments.map((s) => s._id)
    );
    ...
    console.error(
      "ERROR: getBidsOnMyShipments - Error fetching bids on user's shipments:",
      err
    );
    res.status(500).json({
      message: "Error fetching bids on user's shipments",
      error: err.message,
    });
  }
};
...
const getAllBids = async (req, res) => {
  try {
    ...
    const bids = await Bid.find({})
      .populate(
        "carrier",
        "name email role companyName country profilePicture profilePictureUrl phoneNumber phone"
      )
      .populate(
        "shipment",
        "shipmentTitle status pickupAddress deliveryAddress"
...
const markBidAsSeen = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Not authorized to mark this bid as seen" });
    }
    ...
    res
      .status(500)
      .json({ message: "Error marking bid as seen", error: err.message });
  }
};
  ...
  try {
    ...
    try {
      ...
      const notificationData = {
        ...
        message: `The shipper has requested a price update for shipment "${
          bid.shipment.shipmentTitle
        }". They prefer ${currency || bid.currency} ${parseFloat(
          requestedPrice
        ).toLocaleString()}.`,
        priority: "high",
        relatedEntity: {
      ...
      await NotificationService.createNotification(notificationData);
      console.log(
        "✅ Price update request notification created for logistics company"
      );
    } catch (notificationError) {
      console.error(
        "❌ Error creating price update request notification:",
        notificationError
      );
    }
...
const respondToPriceUpdateRequest = async (req, res) => {
  try {
    ...
    if (
      !bid.priceUpdateRequest ||
      bid.priceUpdateRequest.status !== "pending"
    ) {
      return res.status(400).json({
        success: false,
    ...
    try {
      ...
      console.error(
        "❌ Error creating price update response notification:",
        notificationError
      );
    }
