...
exports.verifyLogistics = async (req, res) => {
  try {
    ...
    try {
      console.log(
        "📦 Creating notifications for logistics verification:",
        logistics._id,
        action
      );
      ...
      console.error(
        "❌ Error creating verification notifications:",
        notificationError
      );
...
exports.listDisputes = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch disputes", error: err.message });
  }
};
...
exports.resolveDispute = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to resolve dispute", error: err.message });
  }
};
...
exports.getTotalUsers = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch total users", error: err.message });
  }
};
...
exports.getNormalUsersCount = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Failed to fetch normal users count",
      error: err.message,
    });
  }
};
...
exports.getAllShipments = async (req, res) => {
  try {
    ...
    console.log("DEBUG: getAllShipments - Query params:", {
      search,
      status,
      page,
      limit,
    ...
    console.log(
      "DEBUG: getAllShipments - MongoDB query:",
    ...
    console.log(
      "DEBUG: getAllShipments - Found items:",
      items.length,
      "Total:",
      total
    );
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch shipments", error: err.message });
  }
};
  ...
  try {
    ...
    const [items, total] = await Promise.all([
      Conversation.find()
        .populate(
          "participants",
          "name email role profilePicture companyName contactName contactPosition country yearsInOperation registrationNumber companySize"
        )
        .sort({ updatedAt: -1 })
        .skip(skip)
        .limit(Number(limit)),
...
exports.dashboardSummary = async (req, res) => {
  try {
    const [users, logisticsPending, logisticsVerified, disputes, analytics] =
      await Promise.all([
        User.find().sort({ createdAt: -1 }).select("-password"),
        User.find({ role: "logistics", verificationStatus: "pending" }).sort({
          createdAt: -1,
        }),
        User.find({ role: "logistics", verificationStatus: "verified" }).sort({
          createdAt: -1,
        }),
        Dispute.find()
          .populate("reporter", "name email role")
        ...
        (async () => {
          ...
          // Calculate completed shipments (both completed and delivered statuses)
          const completedShipmentsTotal =
            completedShipments + deliveredShipments;

          console.log(
            "DEBUG - dashboardSummary: Fetched verifiedLogisticsCount:",
            verifiedLogisticsCount
          );
          console.log(
            "DEBUG - dashboardSummary: Fetched pendingLogisticsCount:",
            pendingLogisticsCount
          );
          ...
          return {
            ...
            logistics: {
              total: totalLogistics,
              verified: verifiedLogisticsCount,
              pending: pendingLogisticsCount,
            },
            shipments: {
              total: totalShipments,
              open: openShipments,
    ...
    console.log(
      "DEBUG - dashboardSummary: logisticsPending (users array):",
      logisticsPending
    );
    console.log(
      "DEBUG - dashboardSummary: logisticsVerified (users array):",
      logisticsVerified
    );
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    if (user.role === "admin") {
      ...
      console.log(
        `Attempted to delete admin user with ID: ${id}. Operation forbidden.`
      );
      return res
        .status(403)
        .json({ message: "Cannot delete an administrator account." });
    }
    ...
    for (const conversation of conversations) {
      ...
      conversation.unreadCounts = conversation.unreadCounts.filter(
        (uc) => uc.user && uc.user.toString() !== id
      );

      if (conversation.participants.length === 0) {
        console.log(
          `Conversation ${conversation._id} is now empty. Deleting messages and conversation.`
        );
        await Message.deleteMany({ conversation: conversation._id });
        await conversation.deleteOne(); // Use deleteOne() instead of remove()
      } else {
        console.log(
          `Conversation ${conversation._id} still has participants. Saving changes.`
        );
 ...
 * Body: { status: "active"|"suspended", suspensionEndDate?: Date, suspensionReason?: String }
 */
exports.suspendUser = async (req, res) => {
  try {
    const { id } = req.params;
    const { status, suspensionEndDate, suspensionReason } = req.body;
    ...
    if (user.role === "admin") {
      return res
        .status(403)
        .json({ message: "Cannot suspend/activate an administrator account." });
    }
    ...
    if (status === "suspended") {
      ...
      try {
        ...
        const suspensionDuration = suspensionEndDate
          ? Math.ceil(
              (new Date(suspensionEndDate) - new Date()) / (1000 * 60 * 60 * 24)
            )
          : null;
        ...
        console.log(`✅ Suspension email result:`, result);
      ...
      try {
        console.log(
          `📧 Attempting to send reactivation email to: ${user.email}`
        );
        ...
        console.log(`📧 Reactivation email data:`, emailData);
        ...
        console.log(`✅ Reactivation email result:`, result);
        console.log(
          `✅ Reactivation email sent successfully to: ${user.email}`
        );
      } catch (emailError) {
        console.error("❌ Failed to send reactivation email:", emailError);
        console.error("❌ Reactivation email error details:", {
          message: emailError.message,
          stack: emailError.stack,
          userEmail: user.email,
        });
  ...
  try {
    ...
    if (user.role === "admin" && req.user.id !== user.id) {
      ...
      return res.status(403).json({
        message: "Unauthorized to edit another administrator's account.",
      });
    }
