...
const requireEmailVerification = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(userId).select('email isVerified verificationTokenExpires');
    ...
    if (!user.isVerified) {
      const now = new Date();
      const hasValidToken = user.verificationTokenExpires && user.verificationTokenExpires > now;
      const timeLeft = hasValidToken ? Math.ceil((user.verificationTokenExpires - now) / (1000 * 60)) : 0;
