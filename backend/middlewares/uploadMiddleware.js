...
const storage = multer.diskStorage({
  destination(req, file, cb) {
    ...
    if (file.fieldname === "profilePicture") {
      console.log(
        "📁 Upload Middleware: Storing profile picture in uploads/profiles/"
      );
    ...
    // Store chat attachments in 'uploads/chat'
      ...
      // 'file' is the fieldname used by NewMessageInput for attachments
...
const fileFilter = (req, file, cb) => {
  ...
  if (file.fieldname === "profilePicture" || file.fieldname === "photos") {
    ...
    return cb(
      new Error(
        "Only JPG, JPEG, PNG, AVIF, WebP images are allowed for profile pictures and photos!"
      )
    );
  }
...
const upload = multer({
  storage,
  fileFilter,
  limits: { fileSize: 10 * 1024 * 1024 }, // Limit file size to 10MB (consistent with profile pictures)
