...
const SignupForm = () => {
  ...
  useEffect(() => {
    if (
      ...
      !("registrationNumber" in formData)
    ) {
      setFormData((prev) => ({
        ...prev,
        registrationNumber: "",
  ...
  useEffect(() => {
    if (user?.role) {
      navigate(
        user.role === ROLES.USER ? "/user/dashboard" : "/logistics/dashboard"
      );
    }
  }, [user, navigate]);
  ...
  const validateField = useCallback(
    (name, value, currentFormData, passwordValue) => {
      ...
      if (["agreements", "terms"].includes(name) && !value)
  ...
  const handleChange = useCallback(
    (e) => {
      ...
      setFormData((prevFormData) => {
        ...
        if (
          type === "checkbox" &&
          (name.startsWith("services-") || name.startsWith("regions-"))
        ) {
          ...
          const updatedArray = checked
            ...
            : prevFormData[field].filter(
                (item) => item !== serviceOrRegionValue
              );
    ...
    [validateField]
  ); // Dependency on validateField (now stable)
    ...
    async (e) => {
      ...
      requiredFields.forEach((field) => {
        ...
        if (["services", "regions"].includes(field)) {
          ...
          ["businessLicense", "insuranceCertificate", "governmentId"].includes(
            field
          )
        ) {
          ...
          const msg = validateField(
            ...
            formData[field],
            formData,
            field === "confirmPassword" ? passwordValueForValidation : null
          ); // Pass formData and passwordValue
          if (msg) {
            newErrors[field] = msg;
          } else {
          }
      ...
      try {
        ...
        if (formData.role === ROLES.LOGISTICS) {
          ...
          logisticsFields.forEach((field) => {
            if (field in normalUserData) {
              console.error(
                "❌ ERROR: Logistics field found in normal user data:",
                field
              );
          ...
          if ("registrationNumber" in normalUserData) {
            console.error(
              "❌ CRITICAL: registrationNumber still present in normalUserData after cleanup!"
            );
        ...
        if (signupUser.fulfilled.match(resultAction)) {
          ...
          switch (role) {
            ...
            default:
              console.warn(
                "Unknown role, redirecting to user dashboard:",
                role
              );
              navigate("/user/dashboard");
          }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Connect with the world's leading logistics network. Choose your role
                  ...
                  <button
                    ...
                    className={`w-full py-4 px-6 rounded-xl text-white font-semibold text-lg transition-all duration-300 transform ${
                      loading
                        ...
                        : "bg-blue-500 hover:bg-blue-600 shadow-lg hover:shadow-xl hover:scale-105"
                    }`}
                  >
                    {loading ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 bg-gray-50 border border-gray-200 dark:border-gray-700 rounded-xl">
                        {[
                          ...
                          <label
                            key={service}
                            className="flex items-center text-gray-700 hover:bg-white dark:bg-gray-800 p-2 rounded-lg transition-colors cursor-pointer"
                          >
                            <input
                              type="checkbox"
                      ...
                      <div key={doc.key} className="space-y-2">
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full text-sm text-gray-500 file:mr-4 file:py-3 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-indigo-700 hover:file:bg-blue-100 cursor-pointer border border-gray-300 rounded-xl p-2"
                          />
                        </div>
                        {errors[doc.key] && (
                          <p className="text-red-500 text-sm mt-1">
                  ...
                  <div className="space-y-4 p-6 bg-gray-50 rounded-xl border border-gray-200 dark:border-gray-700">
                    <label className="flex items-start text-gray-700 hover:bg-white dark:bg-gray-800 p-3 rounded-lg transition-colors cursor-pointer">
                      ...
                      <span className="text-sm">
                        ...
                        complete. <span className="text-red-500">*</span>
                      </span>
                    </label>
                    {errors.agreements && (
                      <p className="text-red-500 text-sm mt-1">
                    ...
                    <label className="flex items-start text-gray-700 hover:bg-white dark:bg-gray-800 p-3 rounded-lg transition-colors cursor-pointer">
                      ...
                      />
                      <span className="text-sm">
                        I agree to RoamEase's Terms & Conditions and Privacy
