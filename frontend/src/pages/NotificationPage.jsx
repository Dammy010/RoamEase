...
const NotificationPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const notificationsResult = await dispatch(
          getNotifications({ page: 1, limit: 20, status: filters.status })
        );
  ...
  useEffect(() => {
    ...
    const setupSocket = async () => {
      try {
        const { getSocket: socketFunction } = await import(
          "../services/socket"
        );
  ...
  const handleFilterChange = useCallback(
    (newFilters) => {
      ...
      dispatch(
        getNotifications({ page: 1, limit: 20, ...filters, ...newFilters })
      );
    },
    [dispatch, filters]
  );
  ...
  const handleBulkAction = useCallback(
    async (action) => {
      ...
      try {
        await dispatch(
          bulkAction({ action, notificationIds: selectedNotifications })
        );
      } catch (error) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-gray-900">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            Notifications data is not in the expected format.
          </p>
          <button
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-white/20 backdrop-blur-sm text-white rounded-xl hover:bg-white/30 transition-all duration-300 flex items-center gap-2 border border-white/20 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <RefreshCw
                    size={16}
          ...
          {selectedNotifications.length > 0 && (
            <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4 z-50">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-2">
                  {(() => {
                    const selectedUnreadCount = selectedNotifications.filter(
                      (id) => {
