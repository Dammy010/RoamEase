  ...
  async (shipmentId = null, thunkAPI) => {
    try {
      const url = shipmentId
        ? `/admin/bids?shipmentId=${shipmentId}`
        : "/admin/bids";
...
export const suspendUser = createAsyncThunk(
  "admin/suspendUser",
  async (
    { userId, newStatus, suspensionEndDate, suspensionReason },
    thunkAPI
  ) => {
    try {
...
export const resolveDispute = createAsyncThunk(
  ...
  async ({ disputeId, status, adminNotes, resolution }, thunkAPI) => {
    try {
      ...
      toast.success(
        `Dispute ${status === "resolved" ? "resolved" : "updated"} successfully`
      );
...
const adminSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchAllConversationsForAdmin.fulfilled,
        (state, { payload }) => {
      ...
      .addCase(updateUser.fulfilled, (state, { payload }) => {
        ...
        if (payload.role === "logistics") {
          const pendingIndex = state.logisticsPending.findIndex(
            (user) => user._id === payload._id
          );
          if (pendingIndex !== -1) {
            state.logisticsPending[pendingIndex] = {
              ...state.logisticsPending[pendingIndex],
              ...payload,
            };
          }

          const verifiedIndex = state.logisticsVerified.findIndex(
            (user) => user._id === payload._id
          );
          if (verifiedIndex !== -1) {
            state.logisticsVerified[verifiedIndex] = {
              ...state.logisticsVerified[verifiedIndex],
              ...payload,
            };
          }
      ...
      .addCase(suspendUser.fulfilled, (state, { payload }) => {
        ...
        const logisticsIndexPending = state.logisticsPending.findIndex(
          (user) => user._id === payload._id
        );
        if (logisticsIndexPending !== -1) {
          state.logisticsPending[logisticsIndexPending].isActive =
            payload.isActive;
        }
        const logisticsIndexVerified = state.logisticsVerified.findIndex(
          (user) => user._id === payload._id
        );
        if (logisticsIndexVerified !== -1) {
          state.logisticsVerified[logisticsIndexVerified].isActive =
            payload.isActive;
        }
      })
      ...
      .addCase(resolveDispute.fulfilled, (state, { payload }) => {
        ...
        const index = state.disputes.findIndex(
          (dispute) => dispute._id === payload._id
        );
        if (index !== -1) {
