...
export const CurrencyProvider = ({ children }) => {
  ...
  const formatCurrency = (amount, fromCurrency = 'USD', targetCurrency = currency, options = {}) => {
    ...
    const formattedNumber = convertedAmount.toLocaleString(currentCurrency.locale, {
      minimumFractionDigits,
      maximumFractionDigits
    });

    const formatted = showSymbol ? `${symbol}${formattedNumber}` : formattedNumber;
    

    return formatted;
  };

  const formatCurrencySimple = (amount, fromCurrency = 'USD', targetCurrency = currency) => {
    if (amount === null || amount === undefined || isNaN(amount)) {
      return getCurrencySymbol(targetCurrency) + '0.00';
    }
